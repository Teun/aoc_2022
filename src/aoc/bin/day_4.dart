...
void main(List<String> arguments) async {
  final rig = Rig(4, (raw, {dynamic extra}) async {
    var items =
        parseToObjects(raw, RegExp(r'(\d+)-(\d+),(\d+)-(\d+)'), (matches) {
      return Pair(int.parse(matches[0]), int.parse(matches[1]),
          int.parse(matches[2]), int.parse(matches[3]));
    });
