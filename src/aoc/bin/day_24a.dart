...
void main(List<String> arguments) async {
  final rig = Rig(24, (raw, {extra = 1}) async {
    ...
    var path = pf.findShortest<CoordPlusTime, String>(
        expand, (pos) => pos.coord == end, CoordPlusTime(start, 0),
        minimalDistanceRemaining: (from) =>
            from.coord.manhattan(end).toDouble(),
        progress: log);
    var path2 = pf.findShortest<CoordPlusTime, String>(expand,
        (pos) => pos.coord == start, CoordPlusTime(end, path.cost.toInt()),
        minimalDistanceRemaining: (from) =>
            from.coord.manhattan(start).toDouble(),
        progress: log);
    var path3 = pf.findShortest<CoordPlusTime, String>(
        expand,
        (pos) => pos.coord == end,
        CoordPlusTime(start, (path.cost + path2.cost).toInt()),
        minimalDistanceRemaining: (from) =>
            from.coord.manhattan(end).toDouble(),
        progress: log);

    return path.cost + path2.cost + path3.cost;
  });

  var allOK = await rig.testSnippet("sample", 54);
