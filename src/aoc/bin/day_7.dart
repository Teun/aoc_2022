...
class Dir extends FS {
  ...
  @override
  int size() => subdirs.values
      .cast<FS>()
      .followedBy(files.values)
      .fold(0, (acc, item) => acc + item.size());
  List<Dir> recursiveDirs() {
    List<Dir> all = [this];

    for (var element in subdirs.values) {
      all.addAll(element.recursiveDirs());
    }
    return all;
  }
}
...
void main(List<String> arguments) async {
  final rig = Rig(7, (raw) async {
    ...
    return sizes.where((s) => s <= 100000).fold(0, (acc, s) => acc + s);
  });
